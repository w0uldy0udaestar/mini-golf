...
| `dog` 강아지 (3차) | 생물 | common | 공 정지 | 멍멍 ×2 (0.7s) | 넓은 쪽 화면 밖에서 달려와 공을 문다(공 노드 숨김, 입에 공) → 꼬리를 흔들며 ±4~15m(물 제외·벽 릴리프 46px 안) 지점으로 달려가 떨어뜨린다 → `ball` 갱신·HUD 갱신, 잠깐 앉아 꼬리 흔들고 멍 한 번, 반대쪽으로 퇴장 | 물 때 화들짝 · 달아날 때 훠이훠이 · 가까워지면 낄낄, 멀어지면 처짐 |
