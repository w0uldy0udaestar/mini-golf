...
**v0.4.0 릴리스 완료** — CMU 모캡 골프 의식(티 꽂기·공 줍기, RitualAnim) + 스윙 대조 검증.
v0.3.0(창 범퍼·기록/배지/모자·서프라이즈 8%·바람)까지 누적 배포됨. 테스트 42개 통과.
...
와 Sidecar 모니터 선택(preferredDisplayID=11)·범퍼 꺼짐이 번들 도메인에 남아, v0.4.0이
"무음·안 보이는 화면·범퍼 없음"으로 체감 0이 됐었다. 잔재 3종 삭제로 해결(기록·고대비 보존).
v0.4.1에 주 디스플레이 폴백(origin 0,0) 수정 포함. 교훈: 설정 복구·검증은 반드시 번들
도메인 기준으로.

### ⚠️ 다음 세션 재개 지점 — 무드 워크 (모캡 3단계, 사용자 승인된 계획의 잔여분)
...
- 방법: 성격 걷기 클립과 표준 걷기(neutral)의 채널 차분 → WalkFlavor 오버레이 초안
  자동 생성 → 수동 다듬기. 발 접지 게이트는 절대 불변 — 상체 채널·보폭/듀티 파라미터만
- 참고: docs/research-mocap-index.md '이식 1차 결과' 절
