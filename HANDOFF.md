...
## 현재 상태 (2026-09-24 — v0.8.2 뒤 판정 반영 4건: 걸음 속도·티 페그(8754c3f)·퍼팅 타격감(c171b21)·퍼터 페이스·티 여백(fa5db53). v0.8.3 미배포)
...
1. ~~백로그 4종~~·~~v0.8.2 릴리스~~ 완료. **판정 반영**(8754c3f): "걸음이 어떨 땐 너무 느리다" → 처짐 1.15·험한 길 1.2·합계 상한 1.3 ·
   "티 표현"(사용자 제안) → 티 페그 등장·임팩트에 튕겨 날아감(`teeNode`, 티 박스 첫 샷만) · "퍼터를 맞고 나아가는 느낌이 없다"(c171b21) →
   원인은 스케일 불일치(공은 코스 3px/m, 헤드는 스틱맨 스케일 — 헤드가 공을 뚫고 지나감). 임팩트 0.1s 홀드 + 팔로 0.15+0.85·h +
   출발 킥(렌더 4+3h px, 1s 해소) + 스쿼시·접촉 링·'톡'. 실측 도구: `--demo-trademark` RIG 덤프의 grip·phi·len으로 팁 x, `PUTT impact` 로그.
   판정 3차(fa5db53): "퍼터 헤드면을 맞고 나가는 게 안 보인다" → 페이스가 공 안쪽에 그려지던 기하(팁 = 공 뒤 5px + 헤드 블록 9.75) 수정 —
   손·클럽 각 회전 + ballFwd 10/20, PosesTests 페이스 기준 · "티샷이 화면 끝에 매번 걸린다" → 티 여백 월드 폭 8%(파5 25 → 48m, 52 → 122px).
   v0.8.3 릴리스는 확인 뒤. 남은 백로그: Apple 공증(Developer 계정 필요) ·
